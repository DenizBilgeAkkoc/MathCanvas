...
INTERSECT = [
    ...
    # --- Composite/Complex Constructions ---
    'incenter2',
    'excenter2',
    'centroid',
    'cc_tangent',
    ...
    # --- Points from Distance/Angle Constraints ---
    'circle',
    'eqdistance',
    'eqangle2',
    's_angle',
...
class Graph:
  ...
  def copy(self) -> Graph:
    ...
    g, _, _ = Graph.build_problem(p, definitions, verbose=False, init_copy=False)
  ...
  def build_problem(
  ...
  ) -> tuple[Graph, list[Dependency], list[str]]:
    ...
    while not check:
      ...
      for clause in pr.clauses:
        adds, plevel, caption = g.add_clause(
  ...
  def add_clause(
    ...
    # print('construction:', [c.name for c in clause.constructions])
    ...
    for c in clause.constructions:
      ...
      sub_caption = cdef.description
      for p in cdef.construction.args:
        name = mapping[p].upper()
        if len(name) > 1:
          name = name[0] + '_' + name[1:]
        assert f"<|{p.upper()}|>" in sub_caption, f'<|{p.upper()}|>\n{sub_caption}\n{cdef.description}\n------------'
        sub_caption = sub_caption.replace(f"<|{p.upper()}|>", name)
      assert '<|' not in sub_caption, f'\n{sub_caption}\n{cdef.description}\n------------'
      sub_captions.append(sub_caption.strip())
      if len(sub_captions) == 2:
        cap1, cap2 = sub_captions
        if cap1.split(' on ')[0] == cap2.split(' on ')[0]:
            new_prefix = cap1[:-1].replace(' on ', ' as the intersection of ')
            assert cap2.count(' on ') == 1, f'{cap2}'
            cap2 = cap2.split(' on ')[1]
            sub_captions = [f"{new_prefix} and {cap2}"]
            # print(sub_captions[0])
        else:
          idx = 0
          while cap1[idx] == cap2[idx]:
            idx += 1
          while cap1[idx - 1] != ' ':
            idx -= 1
          sub_captions = [f"{cap1[:-1]} and {cap2[idx:]}"]
      elif len(sub_captions) > 2:
        print(sub_captions)
    ...
    return added, plevel, ' '.join(sub_captions)
