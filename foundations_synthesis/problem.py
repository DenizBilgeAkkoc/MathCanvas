...
class Problem:
  ...
  def txt(self) -> str:
    clauses_text = '; '.join([c.txt() for c in self.clauses])
    goal_text = ' ? ' + self.goal.txt() if self.goal else ''

    result = clauses_text + goal_text
    return result
...
class Definition:
  ...
  def from_txt(cls, data: str) -> Definition:
    ...
    construction, rely, deps, basics, numerics, description, _ = data.split('\n')
