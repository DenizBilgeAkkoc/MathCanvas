...
class Bagel(PreTrainedModel):
    ...
    def forward(
        ...
        if self.config.visual_und:
            if vit_token_seqlens is None or len(vit_token_seqlens) == 0:
                print("Error: vit_token_seqlens is None or empty, but visual_und is enabled.")
    ...
    def forward_cache_update_vae(
        ...
        padded_latent = vae_model.encode(padded_images.to(self.device))
    ...
    def generate_image(
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=not verbose):
    ...
    def generate_text(
        ...
        while step < max_length:
            ...
            # Check if we hit the end token (i.e., <|im_end|>)
            if end_token_id is not None and curr_tokens[0] == end_token_id: # only support batch=1
                ...
                output = self.language_model.forward_inference(
                    ...
                    key_values_lens=key_values_lens,
                    packed_key_value_indexes=packed_key_value_indexes,
                    update_past_key_values=False,  # Don't update cache for this prediction
                    is_causal=True,
                    **extra_inputs,
                )
                ...
                # Always include the next token after <|im_end|>
