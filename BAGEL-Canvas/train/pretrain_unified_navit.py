...
def main():
    ...
    # 只有在enable_wandb为True时才进行wandb登录
    ...
    if dist.get_rank() == 0:
        ...
        # 只有在enable_wandb为True时才初始化wandb
        if training_args.enable_wandb:
            wandb.init(
                project=training_args.wandb_project, 
                id=f"{training_args.wandb_name}--{training_args.wandb_runid}", 
    ...
    if training_args.debug_batches > 0:
        # 只在 Rank 0 上执行调试逻辑
        if dist.get_rank() == 0:
            logger.info(f">>> Starting data debugging for {training_args.debug_batches} batches on RANK 0...")
            debug_loader_iter = iter(train_loader)
            for i in range(training_args.debug_batches):
                try:
                    ...
                    break
            logger.info(">>> Data debugging finished. Proceeding with training...")
        dist.barrier()
    ...
    for curr_step, data in enumerate(train_loader, start=train_step):
        ...
        if curr_step % training_args.log_every == 0:
            ...
            if dist.get_rank() == 0 and training_args.enable_wandb:
        ...
        if curr_step > 0 and curr_step % training_args.save_every == 0:
            ...
            torch.cuda.synchronize()
            dist.barrier()
            logger.info(f"Saving checkpoint at step {curr_step}...")
